...
const fetchToWeb = async function fetchWithHeaders(...args: Params) {
  ...
  const headers = {
    "x-hamza-project-group-id": process.env.EXPO_PUBLIC_PROJECT_GROUP_ID,
    host: process.env.EXPO_PUBLIC_HOST,
    "x-forwarded-host": process.env.EXPO_PUBLIC_HOST,
    "x-hamza-host": process.env.EXPO_PUBLIC_HOST,
  };
  ...
  const auth = await SecureStore.getItemAsync(authKey)
    .then((auth) => {
      ...
      try {
        ...
        console.error("Failed to parse auth from SecureStore:", e);
        return null;
      }
    })
    .catch(() => {
